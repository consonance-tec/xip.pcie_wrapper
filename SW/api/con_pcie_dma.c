...
int pcieio_open_dma_channel(int device_id, int chan_id,int max_num_of_pending_buffers ,bool is_read,struct dma_channel_descriptor *dma_chan_desc)
{
	int fd = -1;
	int rc = 0;
...
int set_max_read_request(int dev_id,int dma_id, uint32_t value)
{
	int ret = -1;
	int fd;
	uint64_t val = dma_id;
	...
	val = (((uint64_t)dma_id) << 32) | value;

	
	printf("set_max_read_request %d %d \n",dev_id, dma_id);
	

	fd = pcieio_open_bar(dev_id, 0);

	if (fd >= 0)
	{
		ret = ioctl(fd, CXF_SET_MAX_READ_REQ, &val);
...
int send_debug_query_req(int fd)
{
	return ioctl(fd, CXF_DEBUG_QUERY, NULL);
...
int pcieio_start_channel(struct dma_channel_descriptor *dma_chan)
{
	...
	if(ret < 0)
		printf("pcieio_start_channel %p   %d %d\n",dma_chan,dma_chan->fd,errno );
	return ret;
}
...
int pcieio_get_num_of_devices()
{
	...
	if (fd)
	{
		...
		if(ioctl(fd, CXF_QUERY_NUM_OF_BOARD, &num_of_devices) >= 0)
			ret = (int)num_of_devices;
		close(fd);
	}	
...
int pcieio_release_interrupt_event(int device, int int_id)
{
	...
	if (fd >= 0)
	{
		...
		if(ret < 0)
			printf("pcieio_release_interrupt_event: iioctl for CXF_CLEAR_USER_INTERRUPT returns error %d\n", ret);
		close(fd);
	}
